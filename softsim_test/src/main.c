...
LOG_MODULE_REGISTER(softsim_working, LOG_LEVEL_INF);
...
{
  char buffer[] = "{\"message\":\"Hello from Onomondo SoftSIM with proper integration!\"}";
...
int main(void)
{
  LOG_INF("SoftSIM sample with proper Onomondo integration (ICCID: 89457300000032481981).");

#ifdef CONFIG_SOFTSIM
  // Explicitly initialize SoftSIM
  int err = nrf_softsim_init();
  if (err) {
    LOG_ERR("Failed to initialize SoftSIM, error: %d", err);
  } else {
    LOG_INF("SoftSIM initialized successfully");
  }
  
  // Check if SoftSIM is provisioned
  bool provisioned = nrf_softsim_check_provisioned();
  LOG_INF("SoftSIM provisioned: %s", provisioned ? "YES" : "NO");
#endif

  int32_t err_modem = nrf_modem_lib_init();
  if (err_modem) {
  ...
  // Select SoftSIM instead of physical SIM
  err = nrf_modem_at_printf("AT%%CSUS=2");
  if (err) {
    LOG_ERR("Failed to select SoftSIM, error: %d", err);
  } else {
    LOG_INF("SoftSIM selected");
  }

  // Configure APN for Onomondo SoftSIM
  err = nrf_modem_at_printf("AT+CGDCONT=1,\"IP\",\"onomondo\"");
  if (err) {
    LOG_ERR("Failed to set APN, error: %d", err);
  } else {
    LOG_INF("APN configured: onomondo");
  }
  ...
  LOG_INF("LTE connected with properly integrated SoftSIM!\n");
  err = server_init();
  if (err) {
